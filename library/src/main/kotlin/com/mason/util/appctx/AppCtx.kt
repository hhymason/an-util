...
import com.mason.util.initializer.AppCtxInitializer
 ...
 * [Initializer] which is initialized by Android as early as possible when your app's
 * process is created.
 */
...
fun Context.canLeakMemory(): Boolean = when (this) {
    ...
    is ContextWrapper -> baseContext === this || baseContext.canLeakMemory()
    else -> applicationContext === null
}

/**
 * This methods is only run if [appCtx] is accessed while [AppCtxInitializer] hasn't been
 * initialized. This may happen in case you're accessing it outside the default process, or in case
 * you are accessing it in a [ContentProvider] with a higher priority than [AppCtxInitializer]
...
@VisibleForTesting(otherwise = VisibleForTesting.PRIVATE)
fun initAndGetAppCtxWithReflection(): Context {
